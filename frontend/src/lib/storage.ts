...
// Save PDF data
export async function savePDF(id: string, pdfData: string): Promise<void> {
  const db = await initDB();
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(STORE_NAME, 'readwrite');
    const store = transaction.objectStore(STORE_NAME);
    const request = store.put(pdfData, id);
...
export async function saveOriginalPDF(id: string, pdfData: string): Promise<void> {
  const db = await initDB();
  return new Promise((resolve, reject) => {
...
export async function getOriginalPDF(id: string): Promise<string | null> {
  const db = await initDB();
  return new Promise((resolve, reject) => {
